...
Remount is the open-source infrastructure for cloud agents. You bring the
harness, whether that is [Claude Code](https://github.com/anthropics/claude-code),
...
or the loop your product runs in Go, Python or TypeScript. Remount gives it a
...
- **Move between local and cloud.** Start an agent on your laptop and hand it
  ...
  from any machine. Claude Code and Codex conversations travel by transcript
  id; every other harness carries its state directory.
...
An agent runtime is the machine an agent works on, plus the control that
keeps that work alive across machines, connections and restarts. You use it to
run agents where the compute is, keep them running when nothing is watching,
pause them where they stand, and get a record of everything they did.
...
- **An agent that lives in both places.** Start it locally where the checkout
  is, move it to a cloud node when it needs hours or a GPU, and bring it back
  to inspect. `remount handoff` and `remount resume` do the move; a task queue
  runs a backlog on a durable timer wherever the workspace happens to be.
...
Two ways to get a Remount control plane and nodes without owning a server,
and one vendor that cannot host nodes yet. Everything below was run, with the
vendor's real API key in the environment, before it went into this file.
...
**ix.dev hosts nodes.** A pool asks the vendor for machines, each machine
enrolls itself as a node with a one-time credential, and the control plane
scales the pool between `--min` and `--max`, destroying machines that sit
idle. Nodes run `--backend gvisor`, which is what a production control plane
admits. Start from a control plane the machines can reach, with a tenant
operator for the pool and a tenant agent for the work:
...
remount server --listen 0.0.0.0:7443 --data /var/lib/remount \
  ...
  --bootstrap-principal root --bootstrap-token-file /run/remount/root.token \
  --provisioners /etc/remount/provisioners.json
export REMOUNT_SERVER=https://control.example REMOUNT_TOKEN="$(cat /run/remount/root.token)"
remount principal create ops --tenant acme --roles operator
remount principal create builder --tenant acme --roles agent
remount token issue ops --tenant acme --role operator --ttl 1h --json     # save access_token
remount token issue builder --tenant acme --role agent --ttl 1h --json   # save access_token
```

`/etc/remount/provisioners.json` names the vendor, the pool it serves, and
where a new machine downloads the node binary from. The API key is an
environment variable the server reads at startup, never a value in the file:
...
```json
{
  ...
  "drivers": [
    {"vendor": "ix", "helper": "/usr/local/bin/remount-ix-helper",
     "token_env": "IX_TOKEN", "region": "us-east-1", "tenant": "acme", "pool": "ix"}
  ]
}
```

Install the vendor CLI next to the server, copy the helper into place, and
create the pool as the tenant operator. Each machine is a fresh VM with KVM,
so `runsc` runs on its KVM platform there; the helper installs the node
binary, `runsc` and a rootfs under the data directory on first boot:

```sh
curl https://ix.dev/install.sh | sh && export IX_TOKEN=...
install -m 0755 scripts/provision/remount-ix-helper /usr/local/bin/
REMOUNT_TOKEN=$OPS_TOKEN remount pool create ix --vendor ix --backend gvisor \
  --min 0 --max 4 --idle-scale-down 15m --region us-east-1 --label zone=ix
```

Then, as the agent principal, ask for a workspace in that zone. The pool
provisions a VM on demand, the node enrolls, the workspace is claimed, and
the machine is destroyed again once it has been idle for the scale-down
window:

```sh
REMOUNT_TOKEN=$BUILDER_TOKEN remount ws create --name hello --label zone=ix --json
REMOUNT_TOKEN=$BUILDER_TOKEN remount exec WS -- sh -c 'uname -r; cat /etc/alpine-release'
# 4.19.0-gvisor
# 3.22.1
REMOUNT_TOKEN=$OPS_TOKEN remount events --type pool,node   # pool.scaled, node.enrolled, pool.retired
```

Measured on 2026-09-06: the workspace was claimed 51 seconds after
`ws create` on a pool that held no machines.

**E2B cannot host nodes yet.** The driver and template in
[`images/e2b/`](images/e2b/) take a sandbox all the way to an enrolled
`gvisor` node, but E2B's kernel is built without the nftables netdev family
and tc flower, which the enforced egress gateway needs, so no isolated
workspace can materialize there. A node started on such a kernel now refuses
to enroll with `enforced network unavailable: nf_tables netdev family is
unsupported by this kernel` instead of accepting work it cannot run. The
details are in the
