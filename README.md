...
Remount is self-hosted infrastructure for cloud agents, and for local agents
that move to the cloud and back. An agent's computer is a **workspace**. It
can run on your laptop, a VM, a GPU box, Kubernetes, Modal or Fly, and it
moves between them with its files, its policy and its saved state. Any
harness works: [Claude Code](https://github.com/anthropics/claude-code),
...
or the agent loop you wrote in Go, Python or TypeScript. Workspaces snapshot,
move, sleep, wake on events, replay their sessions, and never hold an API
key. One static Go binary, one open protocol, Apache-2.0, nothing hosted.
