...
type model struct {
	...
	// Reasons is the proto.Reason* vocabulary. It is generated rather than
	// hand-listed so an SDK's typed-error table can be checked against the
	// same source of truth the Go constants come from.
	Reasons []reason
}
...
func parseModel(root string) (model, error) {
					...
					for i, name := range vs.Names {
						...
						if strings.HasPrefix(name.Name, "Reason") && i < len(vs.Values) {
							if lit, ok := vs.Values[i].(*ast.BasicLit); ok && lit.Kind == token.STRING {
								...
								m.Reasons = append(m.Reasons, reason{Constant: name.Name, Value: value, Doc: doc})
							}
						}
	...
	if len(m.Reasons) == 0 {
		return model{}, errors.New("no proto.Reason* constants found")
	}
	sort.Slice(m.Reasons, func(i, j int) bool { return m.Reasons[i].Value < m.Reasons[j].Value })
...
func renderPython(m model) string {
	...
	b.WriteString("# Reason is the stable sub-classification an error carries beside its code.\n")
	b.WriteString("# remount.errors maps each value to one exception class; an unknown value\n")
	b.WriteString("# degrades to the base ProtocolError rather than to an unhandled case.\n")
	b.WriteString("Reason = Literal[\n")
...
func renderTypeScript(m model) string {
	...
	b.WriteString("// Reason is the stable sub-classification an error carries beside its code.\n")
	b.WriteString("// ./errors.ts maps each value to one error class; an unknown value degrades\n")
	b.WriteString("// to the base ProtocolError rather than to an unhandled case.\n")
	b.WriteString("export type Reason =\n")
	for i, r := range m.Reasons {
		terminator := ""
		if i == len(m.Reasons)-1 {
			terminator = ";"
		}
		fmt.Fprintf(&b, "  | %q%s\n", r.Value, terminator)
	}
