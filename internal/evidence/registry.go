...
var scenarios = []Scenario{
	...
	},
	{
		...
		Recorded: StatusPassed,
		Note:     "exact-host pass. On an Apple Silicon Mac with Colima nested virtualization (handoff-linux-host-2026-09-03.md §2) the gate reports AVAILABLE against real firecracker/jailer v1.16.1, a real guest kernel and an ext4 rootfs, and go test -race -count=10 ./internal/workspace/firecracker is green. The lifecycle was also driven by hand end to end: a workspace reaches claimed on backend=firecracker, exec inside reports guest kernel 5.10.223 against the host's 6.8.0-117-generic, an authoritative checkpoint gives consistency=quiesced at 37 MB, and a workspace created --restore-from that artifact reads the file back. Getting there fixed four defects, three of them in this backend: a handshake that raced the guest boot, a compatibility fence that was unsatisfiable because it contained a timestamp, and a snapshot restore that never remapped the network device. BOUNDED, and the bounds are now measured rather than assumed: prepare-abort, stale generation, incompatible CPU or version, corrupt bundle and disk-full staging are covered by unit tests in this package. A restored guest was shown to CONTINUE rather than reboot on a real host — guest uptime goes 41s before the checkpoint to 42s after the restore. Exactly-once is still unproven, because it is a property of move rather than clone, and `remount ws move` between two Firecracker nodes is blocked by an open defect: Revoke deletes the network devices while the VMM still holds the TAP. The obvious reordering was tried and reverted because it costs a safety invariant; the two-phase design that resolves it is in docs/engineering/gvisor-egress-finding-2026-09-04.md. .github/workflows/kvm.yml still fails its final step deliberately and needs updating by someone who can run it.",
	},
	{
