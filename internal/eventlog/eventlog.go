...
func OpenSQLite(path string) (*SQLite, error) {
	...
	columns := []struct{ name, declaration string }{
		...
		{"session", "TEXT NOT NULL DEFAULT ''"}, {"controller_epoch", "INTEGER NOT NULL DEFAULT 0"},
	}
...
func insertEvent(ctx context.Context, db execer, e *proto.Event) error {
	res, err := db.ExecContext(ctx, `INSERT INTO events(
		...
		observed_at, origin, actor, tenant, workspace, generation, operation_id, producer_seq, session, controller_epoch
	) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		e.At, e.Stream, e.Principal, e.Node, e.Type, e.Payload, e.Cause, e.EventID, e.ReceivedAt,
		e.ObservedAt, e.Origin, e.Actor, e.Tenant, e.Workspace, e.Generation, e.OperationID, e.ProducerSeq, e.Session, e.ControllerEpoch)
	if err != nil {
		return err
	}
...
func (s *SQLite) Read(ctx context.Context, from uint64, stream string, limit int) ([]proto.Event, error) {
	...
	if stream == "" {
		rows, err = s.db.QueryContext(ctx, `SELECT seq, at, stream, principal, node, type, payload, cause,
			event_id, received_at, observed_at, origin, actor, tenant, workspace, generation, operation_id, producer_seq, session, controller_epoch
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&e.Workspace, &e.Generation, &e.OperationID, &e.ProducerSeq, &e.Session, &e.ControllerEpoch,
		); err != nil {
			return nil, err
		}
		out = append(out, e)
